...
def preprocess_image(image_path: str):
    ...
    points = detect_card(image)
    if points is not None:
        print("[OpenCV] Business card contour detected", file=sys.stderr)
    else:
        print("[OpenCV] Card contour not found. Using original image.")
    # Save preview with detected edges
    ...
    if points is not None:
        ...
        image = four_point_transform(image, points.astype("float32"))

    image = enhance_for_ocr(image)
    print("[OpenCV] OCR preprocessing completed", file=sys.stderr)
    UPLOAD_DIR.mkdir(parents=True, exist_ok=True)

    image_name = Path(image_path).stem

    processed_path = str(UPLOAD_DIR / f"{image_name}.opencv.png")
    preview_path = str(UPLOAD_DIR / f"{image_name}.detected.png")

    cv2.imwrite(processed_path, image)
    cv2.imwrite(preview_path, preview)

    print(f"[OpenCV] Edge Detection Preview : {preview_path}", file=sys.stderr)
    print(f"[OpenCV] OCR Image : {processed_path}", file=sys.stderr)
